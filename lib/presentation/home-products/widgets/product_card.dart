...
class ProductCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String selectedImageAsset = ImageAssets.renewable; // Default image

    if (product?.productType == "Renewable Energy") {
      selectedImageAsset = ImageAssets.renewable;
    } else if (product?.productType == "REDD+") {
      selectedImageAsset = ImageAssets.reddPlus;
    } else if (product?.productType == "CORSIA") {
      selectedImageAsset = ImageAssets.corsia;
    } else if (product?.productType == "Gas Abatement") {
      selectedImageAsset = ImageAssets.gas;
    }
    const countriesList = """[
  {
  ...
  },
  {
    ...
    // List<Map<String, dynamic>> countryData = json.decode(countriesList);
    List<Map<String, dynamic>> countryData =
        List<Map<String, dynamic>>.from(json.decode(countriesList));

    // Function to get the country object by name
    Map<String, dynamic>? getCountryByName(String countryName) {
                    ...
                    children: [
                      ...
                      Row(
                          ...
                          children: [
                            ...
                            Text(
                              "${product.discount > 0 ? ' / ' : ""} ${getPriceFormattedWithoutcode(product.priceLocal?.currency ?? product.priceInUsd.currency, product.priceLocal?.price ?? product.priceInUsd.price)}",
                              style: Theme.of(context)
                                  .textTheme
                                  .bodyMedium
                                  ?.copyWith(
                          ...
                          children: [
                            SizedBox(
                                ...
                                child: Flag.fromString(
                                  (getCountryByName(product.country)?[
                                              'countryShortCode']
                                          ?.toString() ??
                                      "US"),
                                  // countryCodesConvertA3ToA2(a3Code: "IN")!,
                ...
                Container(
                  decoration: BoxDecoration(
                      image: DecorationImage(
                          ...
                          fit: BoxFit.contain,
                          // image: AssetImage(ImageAssets.bearButton1))),
                          image:
                              NetworkImage(product.details.standards[0].logo))),
                  // image: AssetImage(product?.details?.standards[0]?.logo))),
                  width: 55.w,
                  height: 35.h,
                ),
                SizedBox(
                ...
                Container(
                  decoration: BoxDecoration(
                      image: DecorationImage(
                          alignment: Alignment.centerRight,
                          fit: BoxFit.contain,
                          image: AssetImage(selectedImageAsset))),
                ...
                children: [
                  ...
                  product.stock <= 0
                      ? Container(
                          ...
                          height: 36.w,
                          child: CircleAvatar(
                            backgroundColor: AppColors.primaryActiveColor,
                            radius: 26.sp,
                            child: Padding(
